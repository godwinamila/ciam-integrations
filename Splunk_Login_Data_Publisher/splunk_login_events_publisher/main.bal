...
function processEventByType(string eventType, json eventData, SecurityEventToken setPayload) returns error? {
    ...
    error? splunkResult = sendEventToSplunk(eventType, eventData, setPayload);
    if splunkResult is error {
        log:printError("Failed to send event to Splunk", splunkResult, eventType = eventType, jwtId = setPayload.jti);
        return splunkResult;
    }

    log:printInfo("Event successfully processed and sent to Splunk", eventType = eventType, jwtId = setPayload.jti);
    return;
}